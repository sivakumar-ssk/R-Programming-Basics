...
#R has several systems for making graphs, but ggplot2 is one of the 
#most elegant and most versatile. ggplot2 implements the grammar of graphics, 
#a coherent system for describing and building graphs. With ggplot2, you can do more faster by learning one system and applying it in many places.

#If you'd like to learn more about the theoretical underpinnings
#of ggplot2 before you start, I'd recommend reading 
...
#To access the datasets, help pages, and functions that we will use in this chapter, 
...
#That one line of code loads the core tidyverse; packages which you will use
#in almost every data analysis. It also tells you which 
#functions from the tidyverse conflict with functions in base R
#(or from other packages you might have loaded).

#If you run this code and get the error message 
#"there is no package called 'tidyverse'", you'll need to first install it, 
#then run library() once again.
...
#For example, ggplot2::ggplot() tells you explicitly that we're using the ggplot() 
...
#mpg contains observations collected by the US Environmental Protection Agency on
#38 models of car.
...
#The plot shows a negative relationship between engine size (displ) and fuel efficiency (hwy).
 ...
 # With ggplot2, you begin a plot with the function ggplot().
  ...
  #but it's not very interesting so I'm not going to show it here.
...
#You complete your graph by adding one or more layers to ggplot(). 
  ...
  #which creates a scatterplot. ggplot2 comes with many geom functions
  #that each add a different type of layer to a plot. 
  #You'll learn a whole bunch of them throughout 
...
#Each geom function in ggplot2 takes a mapping argument. 
  ...
  #and the x and y arguments of aes() specify which variables to
  #map to the x and y axes. ggplot2 looks for the mapped variables in the data argument, 
  #in this case, mpg.
...
#ggplot(data = <DATA>) + 
  ...
  #The rest of the examples we will see how to complete and extend 
  #this template to make different types of graphs. 
...
#In the plot below, one group of points (highlighted in red) seems to fall 
  #outside of the linear trend. These cars have a higher mileage
  #than you might expect. How can you explain these cars?
  
  
  #Let's hypothesize that the cars are hybrids. 
  #One way to test this hypothesis is to look at the class value for each car. 
  #The class variable of the mpg dataset classifies cars into groups such as compact,
  #midsize, and SUV. If the outlying points are hybrids, 
  #they should be classified as compact cars or, perhaps, 
  #subcompact cars (keep in mind that this data was collected before hybrid trucks 
  #and SUVs became popular).

#You can add a third variable, 
  #like class, to a two dimensional scatterplot by mapping it to an aesthetic. 
  #An aesthetic is a visual property of the objects in your plot. 
  #Aesthetics include things like the size, the shape, or the color of your points. 
  #You can display a point (like the one below) in different ways
  #by changing the values of its aesthetic properties. Since we already use 
  #the word "value" to describe data, let's use the word "level" to describe 
  #aesthetic properties. Here we change the levels of a point's size, shape,
  #and color to make the point small, triangular, or blue:
  
  
 # You can convey information about your data by mapping the aesthetics in 
  #your plot to the variables in your dataset. 
...
#a process known as scaling. ggplot2 will also add a 
...
#Sports cars have large engines like SUVs and pickup trucks,
#but small bodies like midsize and compact cars, which improves their gas mileage.
#In hindsight, these cars were unlikely to be hybrids since they have large engines.

#In the above example, we mapped class to the color aesthetic,
...
#which controls the transparency of the points, or to the shape aesthetic, 
#which controls the shape of the points.
...
#with a variable to display. The aes() function gathers together each 
#of the aesthetic mappings used by a layer and passes them to the
#layer's mapping argument. The syntax highlights a useful insight about x and y: 
#the x and y locations of a point are themselves aesthetics, 
#visual properties that you can map to variables to display information about the data.

#Once you map an aesthetic, ggplot2 takes care of the rest.
#It selects a reasonable scale to use with the aesthetic, 
#and it constructs a legend that explains the mapping between levels and values.
...
+ geom_point(mapping = aes(x = displ, y = hwy))
 ...
 # or selecting the function name and pressing F1 in RStudio.
  ...
  #instead skip down to the examples and look for code that matches what you're trying to do.

#If that doesn't help, carefully read the error message.
  
  #Sometimes the answer will be buried there! But when you're new to R, 
  
  #the answer might be in the error message but you don't yet know how to understand it. 
...
#Take the first faceted plot in this section:
 ...
 # When using facet_grid() you should usually put the variable with more unique levels 
 ...
 # Both plots contain the same x variable, the same y variable,
  
  #and both describe the same data. 
...
#A geom is the geometrical object that a plot uses to represent data. 
  ...
  #The plot on the left uses the point geom, and the plot on the
  #right uses the smooth geom, a smooth line fitted to the data.
...
#On the other hand, you could set the linetype of a line. geom_smooth() will 
...
#Here, 4 stands for four-wheel drive,
...
#and extension packages provide even more
...
#which you can find at http://rstudio.com/resources/cheatsheets. 
...
#This, however, introduces some duplication in our code. 
  ...
  #ggplot2 will treat these mappings as global mappings that 
  
  #apply to each geom in the graph. In other words, 
...
# If you place mappings in a geom function, ggplot2 will treat them as
  ...
  #local mappings for the layer. It will use these mappings to 
...
#The local data argument in geom_smooth() overrides the global data argument 
...
#something subtle about plots. Consider a basic bar chart,
#as drawn with geom_bar(). The following chart displays 
#the total number of diamonds in the diamonds dataset, 
#grouped by cut. 
...
#The chart shows that more diamonds are available with high quality 
#cuts than with low quality cuts.
...
#plot the raw values of your dataset. Other graphs,
...
#The figure  describes how this process works with geom_bar().


#You can learn which stat a geom uses by inspecting the 

#default value for the stat argument. For example, 
...
#This works because every geom has a default stat; 
  ...
  #the underlying statistical transformation. There are three reasons you might need to use a stat explicitly:
#You might want to override the default stat. 
  
  #In the code below, I change the stat of geom_bar() from 
  
  #count (the default) to identity. 
  ...
  #they might be referring to this type of bar chart, where the height of the bar is already present in the data, or the previous bar chart where the height of the bar is generated by counting rows.
...
#You might be able to guess at their meaning from the context, 

#and you'll learn exactly what they do soon!)
...
#To find the variables computed by the stat, look for the 
  
  #help section titled "computed variables".
